...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_, prev_state):
        ...
        out_inputs = self.tanh(self.out_gate(torch.cat([input_, prev_state * reset], dim=1)))
...
def check_model():
    in_channels = 3
    model = ConvGRU(input_size=in_channels, hidden_sizes=[32, 64, 16], kernel_sizes=[3, 5, 3], n_layers=3)
    model.eval()
    x = torch.FloatTensor(1, in_channels, 64, 64)
    output = model(x)
