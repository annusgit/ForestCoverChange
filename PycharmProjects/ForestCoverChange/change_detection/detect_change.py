...
def check_difference_map_palsar(args):
    ...
    change_percentage = (second_forest_percentage-first_forest_percentage) * 100 / second_forest_percentage
    print('-> Percentage Forest Gain: {:.3f}%'.format(gain_percentage))
    print('-> Percentage Forest Loss: {:.3f}%'.format(loss_percentage))
    print('-> Percentage Effective Change: {:.3f}%'.format(change_percentage))
